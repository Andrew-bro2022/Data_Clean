...
Synthetic CSV/XLSX files exercise structure, dates, numeric quoting/currency, phantom trailers, totals, unmatched rules, skipped xlsx, and header-detection failure. Paths are wired in `config/file_rules.yaml` under `mappings`. Regenerate files after editing:
