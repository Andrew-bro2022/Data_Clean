...
- **`write_audit_excel(results, output_path)`**  
  - Sheet **`file_summary`**: one row per file — match, header index, row/column counts, `missing_columns` / `extra_columns` (joined strings), layout flags (`column_count_match` / `column_order_match` as `Y`/`N`/blank), issue counts, **`date_issue_columns`** / **`numeric_issue_columns`** (distinct columns with issues in that category), phantom/total flags, **`read_error`**.  
